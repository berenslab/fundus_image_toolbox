...
def circle_crop(img: Union[str, Image.Image, np.ndarray, torch.Tensor, list], size=(512, 512), return_all=False, to_numpy=True):
    ...
    imgs, masks, centers, radii = np.array(imgs), np.array(masks), np.array(centers), np.array(radii)
