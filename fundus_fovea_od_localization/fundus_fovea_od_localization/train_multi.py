...
def get_ensemble(models_dir:str = MODELS_DIR, device:str = "cuda:0", return_test_dataloader:bool = False):
    ens = ["2024-05-07 11:13.05", "2024-05-07 11:13.13"]
    models = []
    checkpoints = []
    for model_dir in [os.path.join(models_dir, x) for x in ens]:
        model, ckpt, test_dataloader = load_model(model_dir, device, return_test_dataloader=True)
        models.append(model)
...
def evaluate_ensemble(models:List[ODFoveaModel], checkpoints:List[str], test_dataloader:ODFoveaLoader):
    test_loss, test_iou, test_dist, standardized_test_dist = [], [], [], []
    for model, ckpt in zip(models, checkpoints):
        loss, iou, dist, std_dist = model.evaluate(test_dataloader, checkpoint_path=ckpt, load_checkpoint=True, save_summary=False)
        test_loss.append(loss)
        test_iou.append(iou)
        test_dist.append(dist)
        standardized_test_dist.append(std_dist)
    
    test_loss, test_iou, test_dist, standardized_test_dist = map(lambda x: sum(x)/len(x), [test_loss, test_iou, test_dist, standardized_test_dist])
...
def load_model(checkpoint_dir:str="default", device:str = None, return_test_dataloader:bool = False):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default=None, help="If passed, other arguments are ignored and the model is trained using the config file")
    parser.add_argument("--model_type", type=str, default="efficientnet-b3", help="Type of model to train", choices=["resnet18", "resnet34", "resnet50", "resnet101", "resnet152", "efficientnet-b0", "efficientnet-b1", "efficientnet-b2", "efficientnet-b3", "efficientnet-b4", "efficientnet-b5", "efficientnet-b6", "efficientnet-b7"])
    parser.add_argument("--device", type=str, default="cuda:0", help="Device to train on")
    parser.add_argument("--epochs", type=int, default=500, help="Number of epochs")
    parser.add_argument("--batch_size", type=int, default=8, help="Batch size")
    parser.add_argument("--lr", type=float, default=1e-4, help="Learning rate")
    parser.add_argument("--img_size", type=int, default=350, help="Size of the input image")
    parser.add_argument("--testset_eval", type=bool, default=True, help="Evaluate on test set after training")
    parser.add_argument("--csv_path", type=str, default="../ADAM+IDRID+REFUGE_df.csv", help="Path to the csv file")
    parser.add_argument("--data_root", type=str, default="../", help="Root folder that the csv entries refer to")
    parser.add_argument("--seed", type=int, default=123, help="Seed for reproducibility. A different fixed seed is used for train-val-test split.")
