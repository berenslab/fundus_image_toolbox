...
class ResizedCenterCropAndPad:
  ...
  def __call__(self, image_label_sample):
    ...
    if self._is_cut(image):
      ...
      assert image.size[1] == new_h, f'Image height should be {new_h}, but is {image.size[1]}'

      # Pad the image top and bottom
      image = tf.pad(image, (0, pad//2, 0, pad//2), padding_mode='constant', fill=0)
      f_y += pad//2
      od_y += pad//2
    ...
    image, (f_label, od_label) = ResizedCenterCrop((self.new_width, self.new_height))((image, (f_label, od_label)))
...
class RandomRotation:
  ...
  def __call__(self, image_label_sample):
    ...
    f_label= (f_x - w/2) * np.cos(np.radians(angle)) - (f_y - h/2) * np.sin(np.radians(angle)) + w/2, (f_x - w/2) * np.sin(np.radians(angle)) + (f_y - h/2) * np.cos(np.radians(angle)) + h/2
    od_label = (od_x - w/2) * np.cos(np.radians(angle)) - (od_y - h/2) * np.sin(np.radians(angle)) + w/2, (od_x - w/2) * np.sin(np.radians(angle)) + (od_y - h/2) * np.cos(np.radians(angle)) + h/2
...
class RandomSqueeze:
  ...
  def __call__(self, image_label_sample):
    ...
    image, (f_label, od_label) = Resize((int(w * x_squeeze), int(h * y_squeeze)))(image_label_sample)
