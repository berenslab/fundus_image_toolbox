...
def any_to_tensor(image):
    ...
    if isinstance(image, str):
        image = to_tensor(PIL.Image.open(image)) # PIL Image with shape (H, W, C) -> (C, H, W)
    elif isinstance(image, np.ndarray):
        image = to_tensor(PIL.Image.fromarray(image)) # numpy array with shape (H, W, C) -> (C, H, W)
    elif isinstance(image, torch.Tensor):
        image = image
    else:
        raise ValueError("Image should be a string, numpy array or torch tensor but is of type", type(image))
    return image
...
def ensemble_predict(ensemble:List[FundusQualityModel], image:Union[list, str, torch.Tensor, np.ndarray], threshold:float=0.5, print_result:bool=False):
    ...
    if isinstance(image, (torch.Tensor, np.ndarray, list)) and isinstance(image[0], (torch.Tensor, np.ndarray, PIL.Image.Image)) and len(image[0].shape) == 3:
    ...
    elif isinstance(image, (list, str)) or isinstance(image, (torch.Tensor,np.ndarray)):
        # Do single image prediction
        preds = []
        for model in ensemble:
            preds.append(model.predict_from_image(image, threshold=None, load_best=False))
...
def ensemble_predict_from_dataloader(ensemble:List[FundusQualityModel], dataloader:DataLoader, threshold:float=0.5, print_result:bool=True):
    ...
    assert len(binary_ensemble_preds) == len(labels) == len(ensemble_preds), "Prediction and label lengths do not match."

    if print_result:
        print(f"Ensemble predicted logits: {ensemble_preds} \nEnsemble predicted classes: {binary_ensemble_preds}, where 1 is good quality)")
