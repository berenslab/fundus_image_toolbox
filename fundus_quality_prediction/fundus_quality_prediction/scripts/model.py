...
def plot_quality(fundus, conf:float, label:int, threshold:float=0.5):
    ...
    fundus = Img(fundus).to_numpy().set_channel_dim(-1).img
    ...
    ax1.imshow(fundus)
    ax1.axis('off')
    ax1.text(0, 20, f'{"Ungradeable" if label == 0 else "Gradeable"}', fontsize=12, color='white', backgroundcolor='black')
...
class FundusQualityModel:
    ...
    def __init__(self, config):
        ...
        self.config = config
        self.model = self._get_model(self.config.model_type) # TODO: Does it use a cuda device automatically if available?
        self.best_epoch = 0
        self.best_loss = float("inf")
        self.best_acc = 0.0
        self.model_selection_metric = self.config.model_selection_metric if hasattr(self.config, "model_selection_metric") else "accuracy"
        ...
        self.checkpoint_path = os.path.join(MODELS_DIR,f"{self.timestamp}/{self.config.model_type}_best.pt")
    ...
    def train(self, train_dataloader, val_dataloader):
        ...
        for epoch in range(self.config.epochs):
            ...
            train_loss, train_acc = self._train_val_step(train_dataloader, self.loss_fn, self.optimizer, pbar)
            ...
            val_loss, val_acc= self._train_val_step(val_dataloader, self.loss_fn, pbar=pbar)
            self.loss_tracker["val"].append(val_loss)
            self.acc_tracker["val"].append(val_acc)

            pbar.set_description(f"Epoch: {epoch+1}, Train Loss: {train_loss:.4f}, Val Loss: {val_loss:.4f}, Train Acc: {train_acc:.4f}, Val Acc: {val_acc:.4f}")
        ...
        print(f"Best epoch: {self.best_epoch}, Best loss: {self.best_loss}, Best acc: {self.best_acc}\n")
    ...
    def _train_val_step(self, dataloader, loss_fn, optimizer=None, pbar=None):
        ...
        for image_batch, label_batch in dataloader:
            ...
            image_batch, label_batch = image_batch.to(self.config.device), label_batch.to(self.config.device)
    ...
    def predict_from_dataloader(self, dataloader, threshold = None, device = None, load_best = True, numpy_cpu=True):
    ...
    def predict_from_image(self, image: Union[str, torch.Tensor, np.ndarray], threshold:float = 0.5, device:str = None, load_best:bool = True, numpy_cpu:bool = True):
        ...
        return self.predict_from_batch(image, threshold, device, load_best, numpy_cpu, transform=False)
    
    def predict_from_batch(self, image_batch: Union[torch.Tensor, np.ndarray], threshold:float = 0.5, device:str = None, load_best:bool = True, numpy_cpu:bool = True, transform=True):
        ...
        if transform:
            image_batch = [get_transforms(split="test")(to_pil_image(image)) for image in image_batch]
    ...
    def evaluate(self, test_dataloader=None, val_dataloader=None, train_dataloader=None, threshold = 0.5, best= True, plot_auc=False):
        if sum(1 for _ in filter(None.__ne__, [test_dataloader, val_dataloader, train_dataloader])) != 1:
        ...
        all_preds, all_labels = self.predict_from_dataloader(dataloader, threshold = threshold, load_best=best, numpy_cpu=False)
    ...
    def save_summary(self, log=None):
        summary_path = os.path.join(MODELS_DIR,f"{self.timestamp}/{self.config.model_type}_summary.txt")
    ...
    def _plot_loss(self, save=False):
        ...
        if save:
            plt.savefig(os.path.join(MODELS_DIR,f"{self.timestamp}/{self.config.model_type}_loss_plot.png"))
