...
WEIGHT_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'SuperRetina/save/SuperRetina.pth')
IMG_SIZE = 512
DEFAULT_CONFIG = {  
                'device': 'cuda:0',
...
def predict_points(fixed_tensor, moving_tensor, config, model):
    ...
    keypoints = [
        torch.nonzero(s > config['nms_thresh'])
        for s in scores]
...
def check_collapse(goodMatch, cv_kpts_fixed, cv_kpts_moving, round_to:int=1, threshold:float=0.65):
    ...
    moving_keypoints = [(round(kp[0], round_to), round(kp[1], 2)) for kp in moving_keypoints]
    fixed_keypoints = [(round(kp[0], round_to), round(kp[1], 2)) for kp in fixed_keypoints]
    ...
    if ratio_moving < threshold or ratio_fixed < threshold:
        raise Exception("Failed to align the two images: Too many points are projected to one!")
    
# %%
def find_homography_and_align(fixed_image, moving_image, original_moving_image, cv_kpts_fixed, cv_kpts_moving, goodMatch, matches, status, show=False, show_mapping=False, save_to=None):
    ...
    if show_mapping:
        ...
        matched_image = get_match_visualization(moving_image, fixed_image, cv_kpts_moving, cv_kpts_fixed, matches, status)
        plt.figure(dpi=300)
    ...
    if H_m is not None:
        ...
        original_moving_align = cv2.warpPerspective(original_moving_image, H_m, (w, h), borderMode=cv2.BORDER_CONSTANT, borderValue=(0))
        ...
        if len(moving_align.shape) == 3:
            ...
            original_moving_align = cv2.cvtColor(original_moving_align, cv2.COLOR_BGR2RGB)
...
def register_one(fixed_image:Union[str, np.ndarray, Image.Image], 
    ...
    if save_to is not None:
        ...
        if isinstance(fixed_image, str) and isinstance(moving_image, str):
            save_to = os.path.join(save_to, os.path.basename(fixed_image).__str__() + "_" + os.path.basename(moving_image).__str__())
        else:
    ...
    fixed_tensor = transform(Image.fromarray(fixed_image), config['model_image_height'], config['model_image_width'])
    moving_tensor = transform(Image.fromarray(moving_image), config['model_image_height'], config['model_image_width'])
    ...
    cv_kpts_moving = [cv2.KeyPoint(int(i[0] / config['model_image_width'] * image_width),
                                int(i[1] / config['model_image_height'] * image_height), 30)  # 30 is keypoints size, which can be ignored
                    for i in moving_keypoints]
    cv_kpts_fixed = [cv2.KeyPoint(int(i[0] / config['model_image_width'] * image_width),
                                int(i[1] / config['model_image_height'] * image_height), 30)
                    for i in fixed_keypoints]

    goodMatch, status, matches = match(matcher, fixed_desc, moving_desc, config)

    moving_aligned = find_homography_and_align(fixed_image, moving_image, original_moving_image, cv_kpts_fixed, cv_kpts_moving, goodMatch, matches, status, show=show, show_mapping=show_mapping, save_to=save_to)
